...
class BertCon(BertPreTrainedModel):
    def __init__(self, bert_config):
        """
        :param bert_config: configuration for the BERT model
        """
        ...
        self.tem = nn.Parameter(torch.tensor(0.05, requires_grad=True))  # Temperature for adaptation
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, sent_labels=None,
        ...
        hidden = outputs[0]  # Last hidden state from BERT
        batch_num = hidden.shape[0]
        w = hidden[:, 0, :]  # Use the embedding of [CLS] token (first token)
        
        h = self.shared_encoder(w)  # Apply the shared encoder

        if meg == 'train':
            h = F.normalize(h, p=2, dim=1)  # Normalize the embeddings

            # Meta-learning loss based on adaptation (no domain labels)
            # Meta-learning loss typically involves computing a task-specific loss here
            if sent_labels is not None:
                sent_labels = sent_labels.unsqueeze(0).repeat(batch_num, 1).T  # Replicate labels for each pair
                rev_sent_labels = sent_labels.T
                rev_h = h.T
                similarity_mat = torch.exp(torch.matmul(h, rev_h) / self.tem)  # Cosine similarity
            ...
            return F.normalize(h, p=2, dim=1)  # Return normalized embeddings for adaptation
    
    # Optional: Include additional functions for meta-learning such as gradient updates for few-shot learning
