...
class BertCon(BertPreTrainedModel):
    def __init__(self, bert_config):
        ...
        # Replacing the shared_encoder with an RNN (LSTM or GRU)
        self.rnn = nn.LSTM(penultimate_hidden_size, 192, batch_first=True, bidirectional=True)

        # Domain classification
        self.dom_loss1 = CrossEntropyLoss()
        self.dom_cls = nn.Linear(192 * 2, bert_config.domain_number)  # Bidirectional RNN, hence 192 * 2
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, sent_labels=None,
        ...
        hidden = outputs[0]  # Output from BERT

        # Pass the BERT output through the RNN
        rnn_out, _ = self.rnn(hidden)  # RNN output (hidden states for all time steps)

        # Use the final output from RNN
        rnn_out = rnn_out[:, -1, :]  # Use last hidden state for classification (batch, hidden_size)
        rnn_out = F.normalize(rnn_out, p=2, dim=1)

        if meg == 'train':
            sent_labels = sent_labels.unsqueeze(0).repeat(hidden.shape[0], 1).T
            rev_sent_labels = sent_labels.T
            rev_rnn_out = rnn_out.T

            similarity_mat = torch.exp(torch.matmul(rnn_out, rev_rnn_out) / self.tem)
            equal_mat = (sent_labels == rev_sent_labels).float()

            eye = torch.eye(hidden.shape[0]).to(similarity_mat.device)
            a = ((equal_mat - eye) * similarity_mat).sum(dim=-1) + 1e-5
            b = ((torch.ones(hidden.shape[0], hidden.shape[0]).to(similarity_mat.device) - eye) * similarity_mat).sum(dim=-1) + 1e-5

            loss = -(torch.log(a / b)).mean(-1)
            return loss

        elif meg == 'source':
            return rnn_out  # Return the normalized RNN output
