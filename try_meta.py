...
def train(args, train_dataset, tokenizer, domain_schema, model):
    """ Train the model with meta-learning adaptation """
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # Forward pass for the current task (current mini-batch)
            loss = model(**inputs)
            ...
            # Meta-update: Accumulate gradients and update model
            tr_loss += loss.item()
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if 0 < args.max_steps < global_step:
                    break  # Stop if max_steps is reached
